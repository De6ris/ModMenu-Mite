...
public class ModListScreen extends GuiScreen {
    ...
    public void initGui() {
        ...
        new ModMenuTexturedButtonWidget(width - 24, paneY, 20, 20, 0, 0, TextureUtils.CONFIGURE_BUTTON_LOCATION, 32, 64) {
            ...
            public void drawButton(Minecraft mc, int mouseX, int mouseY) {
                if (selected != null) {
                    ...
                    enabled = ModMenu.hasConfigScreenFactory(modid) || ModMenu.hasLegacyConfigScreenTask(modid) || Objects.equals(modid, "mite");
                } else {
                    enabled = false;
                }
			...
			final GuiOptions guiOptions = new GuiOptions(this, mc.gameSettings);
            if (screen != null) {
                mc.displayGuiScreen(screen);
            } else {
				LOGGER.warn("Mod {} provides a null screen", modid);
                ModMenu.openConfigScreen(modid);
            }
			if (Objects.equals(modid, "mite")) {
				mc.displayGuiScreen(guiOptions);
			}
        ...
        ModMenuButtonWidget.builder(StringUtils.translate("modmenu.modsFolder"), button -> {
			File modsFolder = FabricUtils.getModsDirectory();
            try {
